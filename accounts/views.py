    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if algorithm_class:
                messages.success(
                    request,
                    f'Algorithm "{algorithm.name}" uploaded and validated successfully!',
                )
            else:
                messages.warning(
                    request,
                    f'Algorithm "{algorithm.name}" uploaded but could not find a TrackingAlgorithm subclass.',
                )
...
def rename_algorithm(request, algorithm_id):
    ...
    return render(
        request,
        "accounts/rename_algorithm.html",
        {
            "form": form,
            "algorithm": algorithm,
        },
...
def save_config(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if (
                config.adjacency_sparsity is not None
                and config.adjacency_sparsity < 100
            ):
            ...
            if is_russian:
                messages.success(
                    request, f'Конфигурация "{config.name}" успешно сохранена!'
                )
            else:
                messages.success(
                    request, f'Configuration "{config.name}" saved successfully!'
                )
    ...
    custom_algorithms = list(
        request.user.algorithms.filter(is_active=True).values_list("name", flat=True)
    )
    all_algorithms = available_algorithms + custom_algorithms

    return render(
        request,
        "accounts/save_config.html",
        {
            "form": form,
...
def delete_config(request, config_id):
    ...
    if is_russian:
        ...
        messages.success(
            request, f'Configuration "{config_name}" deleted successfully!'
        )
    ...
    if request.method == "POST":
        ...
        if not config_ids:
            ...
            if is_russian:
                messages.error(
                    request,
                    "Пожалуйста, выберите хотя бы одну конфигурацию для запуска.",
                )
            else:
                messages.error(
                    request, "Please select at least one configuration to run."
                )
            return redirect("accounts:profile")
        ...
        if not configs:
            ...
            if is_russian:
                messages.error(
                    request, "Не выбрано ни одной действительной конфигурации."
                )
            else:
        ...
        for config in configs:
            params = config.to_params_dict()
            all_params.append({"name": config.name, "params": params})
